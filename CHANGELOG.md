...
- [RFC 0013](rfcs/0013-evaluator-error-and-precedence-cases.md) is `Accepted`, and its five rows —
  ...
  after `0.2.0-draft`. They are in no corpus: the `0.2.0-draft` corpus is frozen and unchanged, and
  no claim may cite a staged row. Its four open questions are decided (a structural fixture that
  fails for exactly one reason; no asserted phase, because §8.4 requires a class and not a phase;
  two fixtures; five rows). The repository's check that every evidence key a case supplies be
  declared by its pack becomes conditional on the expected error class and stricter than before:
  the class a case expects must be the one §8.4's fixed order reports for the case's own inputs,
  read without an evaluator. Every released row passes it unchanged. The RFC was first merged
  without the cross-vendor review the interim regime requires; the review on the accepting pull
  request is its first, and the adoption record says so.
