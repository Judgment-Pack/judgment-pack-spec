...
def coding_vs_business() -> str:
    """The README's comparison, reduced to the single contrast it exists to make.

    The scanned original carries sixteen labelled sub-illustrations. Beside a
    slide's bullets there is room for one idea, so this keeps the idea — one
    environment returns a verdict, the other returns nothing — and drops the
    inventory that supported it.
    """
    pen = Pen(151)
    p = []
    p.append(Text(300, 26, "One of these gets graded", 15, "700").render())
    ...
    panels = (
        {
            ...
            "lines": ("the target is explicit", "the verdict is objective"),
        },
        {
            "x": 312,
            "fill": None,
            ...
            "lines": ("the goal is fuzzy", "feedback comes months later"),
        },
    )
    ...
    for panel in panels:
        ...
        mid = left + 134
        if panel["fill"]:
            p.append(wash(rough_region(pen, left, 44, 268, 232, 3.4), panel["fill"]))
        p.append(ink(rough_rect(pen, left, 44, 268, 232, 3.4), 2.2))
        p.append(Text(mid, 76, panel["title"], 15, "700").render())
        p.append(ink(underline(pen, mid - 62, 86, 124), 1.3, MUTED))

        if panel["tick"]:
            p.append(ink(_tick(pen, mid, 132, 30), 3.4, "#236232"))
        else:
            p.append(Text(mid, 148, "?", 46, "700", fill="#8d3026").render())

        p.append(Text(mid, 190, panel["verdict"], 13, "600").render())
        for index, line in enumerate(panel["lines"]):
            p.append(Text(mid, 220 + index * 22, line, 12, "400", fill=MUTED).render())

    p.append(Text(300, 306, "One environment checks the work. The other cannot.", 13, "700", fill=MUTED).render())
    ...
    return document(
        ...
        desc=(
            ...
            "explicit and the verdict is objective. A business agent gets a question mark: no "
            "verdict at all — the goal is fuzzy and feedback comes months later. One environment "
            "checks the work; the other cannot."
        ),
        body="\n".join("  " + part for part in p),
    )
